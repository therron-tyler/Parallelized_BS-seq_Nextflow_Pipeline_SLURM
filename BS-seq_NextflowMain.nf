...
process CallDMLsDMRs {
    ...
    input:
        path bsseq_file
    output:
        path "${params.dml_out_prefix}*_significant_DMRs.csv", emit: dml_dmr_ch
 
    script:
    """
    
    # v1.1 - tweaked DMR calling R script - loosen DMR criteria, Grp2 - Grp1 (ref) = hypermethylated experimental condition (+)
...
process AnnotateDMRs {
    ...
    input:
        path dmr_csv
    output:
        path "${dmr_csv.baseName}_${params.annotation_outfile}.csv", emit: annotated_ch
    script:
    """
    ...
    Rscript ${params.annotation_script} \
           ...
           --out ${dmr_csv.baseName}_${params.annotation_outfile}
    """
}
